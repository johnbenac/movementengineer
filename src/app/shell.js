...
const SHELL_KEY = '__MODULE_SHELL__';
...
function getTabNameFromEl(el) {
  ...
  if (href && href.startsWith('#') && href.length > 1) {
    return normaliseName(href.slice(1));
  }

  const controls = el.getAttribute?.('aria-controls');
  if (controls) {
    return normaliseName(controls.replace(/-panel$/, '').replace(/^tab-/, ''));
  }

  const id = el.id;
  if (id) {
    return normaliseName(id.replace(/-tab$/, '').replace(/^tab-/, ''));
  }
...
function getPanelNameFromEl(panelEl) {
  ...
  const id = panelEl.id;
  if (id) {
    return normaliseName(id.replace(/-panel$/, '').replace(/^tab-/, ''));
  }
...
export function initShell(ctx, options = {}) {
  if (movementEngineerGlobal[SHELL_KEY]) {
    if (ctx) {
      ctx.shell = movementEngineerGlobal[SHELL_KEY];
    }
    return movementEngineerGlobal[SHELL_KEY];
  }
  ...
  function destroy() {
    ...
    if (movementEngineerGlobal[SHELL_KEY] === api) {
      delete movementEngineerGlobal[SHELL_KEY];
    }
  }
