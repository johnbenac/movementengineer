...
export function initShell(ctx, options = {}) {
  if (movementEngineerGlobal.__moduleShell) {
    if (ctx) {
      ctx.shell = movementEngineerGlobal.__moduleShell;
      ctx.actions = ctx.actions || {};
      if (!ctx.actions.activateTab) {
        ctx.actions.activateTab = movementEngineerGlobal.__moduleShell.activateTab;
      }
      if (!ctx.actions.renderActiveTab) {
        ctx.actions.renderActiveTab = movementEngineerGlobal.__moduleShell.renderActiveTab;
      }
    }
    return movementEngineerGlobal.__moduleShell;
  }
  ...
  movementEngineerGlobal.__moduleShell = shell;
  if (ctx) {
    ctx.shell = shell;
    ctx.actions = ctx.actions || {};
    if (!ctx.actions.activateTab) ctx.actions.activateTab = activateTab;
    if (!ctx.actions.renderActiveTab) ctx.actions.renderActiveTab = renderActiveTab;
  }
