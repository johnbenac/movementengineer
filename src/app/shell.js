...
function normalizeName(name) {
  ...
  return name.replace(/^#/, '').replace(/^tab-/, '').replace(/-panel$/, '').replace(/-tab$/, '');
}

function getTabNameFromEl(el) {
  if (!el) return null;
  const ds = el.dataset || {};
  const direct = ds.tab || ds.tabName || ds.name || ds.target || ds.panel;
  if (direct) return normalizeName(direct);

  const href = el.getAttribute?.('href');
  if (href && href.startsWith('#') && href.length > 1) return normalizeName(href.slice(1));

  const controls = el.getAttribute?.('aria-controls');
  if (controls) return normalizeName(controls);

  if (el.id) return normalizeName(el.id);
...
function getPanelNameFromEl(panelEl) {
  ...
  const direct = ds.tabPanel || ds.tab || ds.name;
  if (direct) return normalizeName(direct);

  if (panelEl.id) return normalizeName(panelEl.id);

  return null;
}

export function initShell(ctx, options = {}) {
  if (movementEngineerGlobal.moduleShell) return movementEngineerGlobal.moduleShell;
  ...
  function getActiveTabName() {
    ...
    const hash = normalizeName(window.location.hash);
  ...
  async function renderActiveTab() {
    ...
    const prevTabName = currentTabName;
    currentTabName = nextName;

    if (prevTabName && prevTabName !== nextName) {
      const prevTab = ctx?.tabs?.[prevTabName];
      if (prevTab?.unmount) {
        try {
  ...
  function destroy() {
    ...
    if (movementEngineerGlobal.moduleShell === api) {
      movementEngineerGlobal.moduleShell = null;
    }
  }

  const api = { getActiveTabName, activateTab, renderActiveTab, destroy };
  movementEngineerGlobal.moduleShell = api;
  if (ctx) ctx.shell = api;
  return api;
}
