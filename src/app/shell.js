function getTabNameFromEl(el) {
  ...
  if (controls) return controls.replace(/-panel$/, '').replace(/^tab-/, '');
...
function getPanelNameFromEl(panelEl) {
  ...
  const id = panelEl.id;
  if (id) return id.replace(/-panel$/, '').replace(/^tab-/, '');
...
export function initShell(ctx, options = {}) {
  ...
  if (typeof window !== 'undefined' && window.__MODULE_SHELL_INITIALIZED__) {
    return window.__MODULE_SHELL_INSTANCE__;
  }
  ...
  if (typeof window !== 'undefined') {
    ...
    window.__MODULE_SHELL_INSTANCE__ = api;
  }
