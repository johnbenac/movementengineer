export function createActions(ctx) {
  ...
  function pushNavigation(nav, collectionName, itemId) {
    if (!collectionName || !itemId) return getNavigation({ navigation: nav });
  ...
  actions.openInCollections = (collectionName, itemId, options = {}) => {
    ...
    if (!Array.isArray(coll)) {
      ctx?.setStatus?.('Unknown collection');
      return false;
    }
    const exists = coll.find(it => it.id === id);
    ...
    ctx?.store?.setState?.(prev => ({
      ...
      navigation: addToHistory
        ? pushNavigation(prev?.navigation, collection, id)
        : prev?.navigation
    }));
  ...
  actions.openChipTarget = target => {
    console.warn('actions.openChipTarget is deprecated; use actions.openTarget instead.');
    return actions.openTarget?.(target);
  };
  actions.jumpToReferencedItem = (collection, id, options) => {
    console.warn(
      'actions.jumpToReferencedItem is deprecated; use actions.openItem instead.'
    );
    return actions.openItem?.(collection, id, options);
  };
  actions.jumpToPractice = id => {
    console.warn('actions.jumpToPractice is deprecated; use actions.openItem instead.');
    return actions.openItem?.('practices', id);
  };
  actions.jumpToEntity = id => {
    console.warn('actions.jumpToEntity is deprecated; use actions.openItem instead.');
    return actions.openItem?.('entities', id);
  };
  actions.jumpToTextCollection = id => {
    console.warn(
      'actions.jumpToTextCollection is deprecated; use actions.openItem instead.'
    );
    return actions.openItem?.('textCollections', id);
  };
  actions.jumpToText = id => {
    console.warn('actions.jumpToText is deprecated; use actions.openItem instead.');
    return actions.openItem?.('texts', id);
  };
