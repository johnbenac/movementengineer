...
const renderLibraryHint = (target, text) =>
  renderHint(target, text, { extraClasses: ['library-empty'] });
...
export function renderLibraryView(ctx) {
  ...
  if (!selection.currentMovementId) {
    renderLibraryHint(shelfList, HINT_TEXT.MOVEMENT_REQUIRED);
    renderLibraryHint(bookList, 'Choose a movement to see books.');
    renderLibraryHint(tocTree, 'No table of contents to show.');
    renderLibraryHint(textEditor, 'Select a movement to edit texts.');
    return;
  }

  const ViewModels = getViewModels(ctx);
  if (!ViewModels || typeof ViewModels.buildLibraryEditorViewModel !== 'function') {
    renderLibraryHint(shelfList, HINT_TEXT.VIEWMODELS_MISSING);
    renderLibraryHint(bookList, HINT_TEXT.VIEWMODELS_MISSING);
    renderLibraryHint(tocTree, HINT_TEXT.VIEWMODELS_MISSING);
    renderLibraryHint(textEditor, HINT_TEXT.VIEWMODELS_MISSING);
    return;
  }
