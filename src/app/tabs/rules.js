...
function renderRulesTab(ctx) {
  ...
  if (!ViewModels?.buildRuleExplorerViewModel) {
    clear(wrapper);
    wrapper.appendChild(hint('ViewModels module not loaded.'));
    return;
  }

  const allRules = (snapshot.rules || []).filter(r => r.movementId === currentMovementId);
  const kinds = Array.from(new Set(allRules.map(r => r.kind).filter(Boolean))).sort((a, b) =>
  ...
  const domainFilter = parseDomainCsv(domainInput.value);
