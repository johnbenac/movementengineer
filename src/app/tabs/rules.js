...
function ensureRulesShell(ctx) {
  const tabManager = ctx?.tabManager;
  if (!tabManager) return;
  tabManager.ensureTab({ id: 'rules', label: 'Rules', group: 'collection' });
  const body = tabManager.getPanelBodyEl?.('rules');
  if (!body) return;
  if (document.getElementById('rules-table-wrapper')) return;
