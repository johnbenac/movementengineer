...
function renderPracticesTab(ctx) {
  ...
  const disableControls = disabled => controls.forEach(el => (el.disabled = !!disabled));
  ...
  title.textContent = vm.practice.name + (vm.practice.kind ? ` (${vm.practice.kind})` : '');
  ...
  if (vm.entities && vm.entities.length) {
    mkSection('Involves entities', section => {
      ...
      vm.entities.forEach(e => {
        ...
        chip.title = e.kind || '';
        if (typeof actions.jumpToEntity === 'function') {
  ...
  if (vm.instructionsTexts && vm.instructionsTexts.length) {
    mkSection('Instruction texts', section => {
      ...
      vm.instructionsTexts.forEach(t => {
        ...
        if (typeof actions.jumpToText === 'function') {
