...
function renderEntitiesTab(ctx, runtime = {}) {
  ...
  const relationTypesInput = document.getElementById('entity-graph-relation-types');
  ...
  if (!select || !detailContainer || !graphDepthSelect || !graphContainer || !relationTypesInput) return;

  const controls = [select, graphDepthSelect, relationTypesInput, refreshBtn].filter(Boolean);

  const disable = () => controls.forEach(el => (el.disabled = true));
  const enable = () => controls.forEach(el => (el.disabled = false));
  ...
  if (!EntityGraphView || typeof ViewModels.buildEntityGraphViewModel !== 'function') {
    graphContainer.appendChild(hint('Graph renderer not available.'));
    return;
  }

  const depth = parseInt(graphDepthSelect.value, 10);
  const relTypesRaw = relationTypesInput.value || '';
  ...
  if (!entityGraphView) {
    entityGraphView = new EntityGraphView({
      onNodeClick: id => {
        if (!id) return;
        select.value = id;
        if (typeof runtime.rerender === 'function') {
          runtime.rerender();
        } else {
          renderEntitiesTab(ctx, runtime);
        }
      }
    });
  }
...
export function registerEntitiesTab(ctx) {
  const tab = {
    ...
    mount(context) {
      ...
      if (graphDepthSelect) graphDepthSelect.addEventListener('change', rerender);
      if (relationTypesInput) relationTypesInput.addEventListener('input', rerender);
    ...
    render(context) {
      const rerender = this.__handlers?.rerender || (() => tab.render(context));
      renderEntitiesTab(context, { rerender });
    },
    unmount() {
      const h = this.__handlers;
      if (!h) return;
      if (h.select) h.select.removeEventListener('change', h.rerender);
      if (h.graphDepthSelect) h.graphDepthSelect.removeEventListener('change', h.rerender);
      if (h.relationTypesInput) h.relationTypesInput.removeEventListener('input', h.rerender);
