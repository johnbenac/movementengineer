...
function renderSchemaGuide(ctx, state, guide, issues) {
  ...
  if (guide?.unknown) {
    ...
    title.textContent = state.currentCollectionName || 'Schema guide';
    header.appendChild(title);
    el.appendChild(header);
    const note = document.createElement('p');
    note.className = 'muted';
    note.textContent = 'Schema guide unavailable for this collection.';
    el.appendChild(note);
    return;
  }
...
function renderCollectionsTab(ctx, tab) {
  ...
  const guide = getSchemaGuide(ctx, collectionName, normalizedState.currentMovementId);
...
function saveCurrentItem(ctx, tab, options = {}) {
  ...
  applyState(ctx, nextState);
  if (persist) {
    ctx.persistence.commitSnapshot(snapshot, { dirtyScope: 'item', save: true });
  } else {
