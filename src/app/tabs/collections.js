...
function renderSchemaGuide(ctx, state, guide, issues) {
  ...
  if (guide.referenceFields.length) {
    ...
    refs.innerHTML =
      ...
      guide.referenceFields.map(r => `<code>${r.field}</code> → <code>${r.target}</code>`).join(' · ');
    el.appendChild(refs);
  }
...
export function registerCollectionsTab(ctx) {
  ...
  mount(context) {
    ...
    const editor = document.getElementById('item-editor');
    const copyBtn = document.getElementById('btn-copy-item-markdown');
    ...
    unmount() {
      ...
      if (h.copyBtn) h.copyBtn.removeEventListener('click', h.handleCopyMarkdown);
