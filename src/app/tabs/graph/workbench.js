...
function getUi(ctx) {
  return ctx?.ui || ctx?.services?.ui || movementEngineerGlobal.ui || {};
}

function setStatus(ctx, text) {
  const ui = getUi(ctx);
  if (ui?.setStatus) return ui.setStatus(text);
  if (ctx?.setStatus) return ctx.setStatus(text);
  if (ctx?.store?.setStatus) return ctx.store.setStatus(text);
  return null;
}

function applyState(ctx, updater) {
  if (typeof ctx?.update === 'function') return ctx.update(updater);
  if (ctx?.store?.update) return ctx.store.update(updater);
...
function setWorkbenchSelection(ctx, selection) {
...
function ensureGraphWorkbenchDom(ctx) {
  ...
  function attachResize(handleEl, side) {
    ...
    handleEl.addEventListener('mousedown', e => {
      ...
      const currentState = getWorkbenchState(ctx);
      const startLeft = currentState.leftWidth;
      const startRight = currentState.rightWidth;
  ...
  dom.clearBtn.addEventListener('click', () => {
    setWorkbenchSelection(ctx, null);
  });
  ...
  dom.createEntityForm.addEventListener('submit', e => {
    ...
    if (!snapshot || !currentMovementId || !DomainService?.addNewItem) return;
    ...
    try {
      applyState(ctx, prev => {
        ...
        const entity = DomainService.addNewItem(snap, 'entities', currentMovementId);
        entity.name = name;
        entity.kind = kind;
        ...
        entity.notes = notes;
        DomainService.upsertItem(snap, 'entities', entity);
        created = entity;
      ...
      if (created?.id) {
        patchWorkbenchState(ctx, {
          selection: { type: 'entity', id: created.id },
          filterCenterId: created.id,
          focusEntityId: created.id
        });
      }
    } catch (err) {
...
function renderGraphSearch(ctx, dom, baseGraphNodes) {
  ...
  getClear(ctx)(dom.searchResults);
...
function renderGraphWorkbenchFilters(ctx, dom, baseGraph) {
  ...
  nodeTypes.forEach(type => {
    ...
    cb.addEventListener('change', () => {
      ...
      patchWorkbenchState(ctx, { filterNodeTypes: next });
    });
...
function renderGenericNodeEditor(ctx, dom, node, config) {
  ...
  btnSave.addEventListener('click', () => {
    ...
    try {
      applyState(ctx, prev => {
        const snap = prev.snapshot || {};
        DomainService.upsertItem(snap, config.collection, updated);
        return { ...prev, snapshot: snap };
      });
      ctx.store?.saveSnapshot?.({ show: false });
  ...
  btnDelete.addEventListener('click', () => {
    ...
    try {
      applyState(ctx, prev => {
        const snap = prev.snapshot || {};
        DomainService.deleteItem(snap, config.collection, item.id);
        return { ...prev, snapshot: snap };
      });
      setWorkbenchSelection(ctx, null);
      ctx.store?.saveSnapshot?.({ show: false });
...
function renderSelected(ctx, dom, visibleEntities, entityById, baseGraph) {
  ...
  const state = getState(ctx);
  const snapshot = state.snapshot || {};
  const selection = getWorkbenchState(ctx).selection;
  if (!selection) {
  ...
  if (selectionType === 'entity') {
    ...
    btnSave.addEventListener('click', () => {
      ...
      try {
        applyState(ctx, prev => {
          const snap = prev.snapshot || {};
          DomainService.upsertItem(snap, 'entities', updated);
          return { ...prev, snapshot: snap };
        });
        ctx.store?.saveSnapshot?.({ show: false });
    ...
    btnDelete.addEventListener('click', () => {
      ...
      try {
        applyState(ctx, prev => {
          const snap = prev.snapshot || {};
          DomainService.deleteItem(snap, 'entities', entity.id);
          return { ...prev, snapshot: snap };
        });
        setWorkbenchSelection(ctx, null);
        ctx.store?.saveSnapshot?.({ show: false });
...
export function renderGraphWorkbench(ctx) {
  ...
  if (!ViewModels?.buildMovementGraphModel || !ViewModels?.filterGraphModel) {
    clear(root);
    const p = document.createElement('p');
    p.className = 'hint';
  ...
  if (workbenchState.filterNodeTypes?.length) {
    const filteredTypes = workbenchState.filterNodeTypes.filter(t => nodeTypes.has(t));
  ...
  if (workbenchState.filterCenterId && !baseNodeIds.has(workbenchState.filterCenterId)) {
    patchWorkbenchState(ctx, { filterCenterId: null, focusEntityId: null });
    return;
  }
  ...
  const selectedType = normaliseSelectionType(
    workbenchState.selection && workbenchState.selection.type
  );
...
export function unmountGraphWorkbench() {
  // Intentionally left blank; DOM cache is retained between mounts.
}
