...
function getGraphDatasetForMovement(ctx) {
  ...
  return { snapshot, movementId, visibleEntities, entityById };
}
...
function ensureGraphWorkbenchDom(ctx) {
  ...
  root.innerHTML = `
    <div id="graph-workbench" class="graph-workbench"
         style="--graph-left-width:${state.leftWidth}px; --graph-right-width:${state.rightWidth}px;">
    ...
    const patch =
      raw === ''
        ...
        : (() => {
            ...
            return { filterDepth: Number.isFinite(n) && n >= 0 ? n : null };
          })();
  ...
  dom.createEntityForm.addEventListener('submit', e => {
    e.preventDefault();
    const { snapshot, movementId } = getGraphDatasetForMovement(ctx);
...
function renderGraphSearch(ctx, dom, baseGraphNodes) {
  ...
  const prev = dom.searchKind.value || state.searchKind || 'all';
  ...
  if (dom.searchQuery.value !== (state.searchQuery || '')) {
    dom.searchQuery.value = state.searchQuery || '';
  }
  ...
  const selType = normaliseSelectionType(state.selection?.type);
  const selectedNodeId = selType && selType !== 'relation' && selType !== 'edge' ? state.selection.id : null;
...
function renderGraphWorkbenchFilters(ctx, dom, baseGraph) {
  ...
  const validNodeTypes = state.filterNodeTypes.filter(t => nodeTypes.includes(t));
  if (validNodeTypes.length !== state.filterNodeTypes.length) {
    patchWorkbenchState(ctx, { filterNodeTypes: validNodeTypes });
    return;
  }

  if (state.filterCenterId && !nodeMap.has(state.filterCenterId)) {
    patchWorkbenchState(ctx, { filterCenterId: null });
    return;
  }

  const centerNode = state.filterCenterId ? nodeMap.get(state.filterCenterId) : null;
  ...
  if (dom.filterDepth) {
    const desired =
      state.filterDepth === null || state.filterDepth === undefined ? '' : String(state.filterDepth);
  ...
  const selectedTypes = new Set(state.filterNodeTypes);
  ...
  nodeTypes.forEach(type => {
    ...
    cb.addEventListener('change', () => {
      const current = getWorkbenchState(ctx).filterNodeTypes || [];
      if (cb.checked) {
        if (!current.includes(type)) {
          patchWorkbenchState(ctx, { filterNodeTypes: current.concat(type).filter(Boolean) });
        }
      } else {
        patchWorkbenchState(ctx, { filterNodeTypes: current.filter(t => t !== type) });
      }
      renderGraphWorkbench(ctx);
    });
...
function renderGenericNodeEditor(ctx, dom, node, config, snapshot) {
  ...
  config.fields.forEach(field => {
    ...
    if (field.name === 'content') {
      ...
      const mdActions = document.createElement('div');
      mdActions.className = 'markdown-row-actions';
      const openBtn = document.createElement('button');
      openBtn.type = 'button';
      openBtn.textContent = 'Open markdown editor';
      mdActions.appendChild(openBtn);
      headerRow.appendChild(mdActions);
