...
const GRAPH_NODE_EDIT_CONFIG = {
  ...
  practice: {
    ...
    fields: [
      ...
      { label: 'Instructions text IDs (csv)', name: 'instructionsTextIds', type: 'csv' },
  ...
  event: {
    ...
    fields: [
      ...
      { label: 'Name', name: 'name' },
      { label: 'Description', name: 'description', type: 'textarea', rows: 3, nullable: true },
      { label: 'Recurrence', name: 'recurrence' },
      { label: 'Timing rule', name: 'timingRule' },
      { label: 'Tags (csv)', name: 'tags', type: 'csv' },
...
function clearElement(ctx, el) {
  if (!el) return;
  const helper = getDom(ctx)?.clearElement;
...
function getWorkbenchState(ctx) {
  ...
  const wb = state.graphWorkbenchState;
  if (wb) return wb;
  return { ...DEFAULT_GRAPH_WORKBENCH_STATE };
}
...
function patchWorkbenchState(ctx, patch) {
  ...
  const updater = prev => ({
    ...(prev || {}),
    graphWorkbenchState: { ...(prev?.graphWorkbenchState || DEFAULT_GRAPH_WORKBENCH_STATE), ...patch }
  });
  if (typeof store.update === 'function') {
    return store.update(updater);
  }
  if (typeof store.setState === 'function') {
    const prev = store.getState ? store.getState() : {};
    return store.setState(updater(prev));
  }
  return getWorkbenchState(ctx);
}
...
function getGraphDatasetForMovement(ctx) {
  const state = getState(ctx);
  const snapshot = state.snapshot || {};
  const currentMovementId = state.currentMovementId;
  const allEntities = normaliseArray(snapshot.entities);
  const visibleEntities = allEntities.filter(e => e && e.movementId === currentMovementId);
  const entityById = buildEntityIndex(visibleEntities);
  return { visibleEntities, entityById };
}

function ensureGraphWorkbenchDom(ctx) {
  const wbState = getWorkbenchState(ctx);
  const root = document.getElementById('graph-workbench-root');
  if (!root) return null;

  if (graphWorkbenchDom && graphWorkbenchDom.root === root) {
    graphWorkbenchDom.workbench?.style?.setProperty('--graph-left-width', `${wbState.leftWidth}px`);
    graphWorkbenchDom.workbench?.style?.setProperty('--graph-right-width', `${wbState.rightWidth}px`);
    return graphWorkbenchDom;
  }

  root.innerHTML = `
    <div id="graph-workbench" class="graph-workbench"
         style="--graph-left-width:${wbState.leftWidth}px; --graph-right-width:${wbState.rightWidth}px;">
  ...
  function attachResize(handleEl, side) {
    ...
    handleEl.addEventListener('mousedown', e => {
      ...
      const startLeft = getWorkbenchState(ctx).leftWidth;
      const startRight = getWorkbenchState(ctx).rightWidth;
  ...
  dom.filterDepth?.addEventListener('input', () => {
    ...
    const patch = {};
    if (raw === '') {
      patch.filterDepth = null;
    } else {
      const n = parseInt(raw, 10);
      patch.filterDepth = Number.isFinite(n) && n >= 0 ? n : null;
    }
    patchWorkbenchState(ctx, patch);
    renderGraphWorkbench(ctx);
  });
  ...
  dom.createEntityForm?.addEventListener('submit', e => {
    ...
    try {
      ...
      store?.saveSnapshot?.({ show: false });
      ui?.setStatus?.('Entity created');
...
function renderGraphSearch(ctx, dom, baseGraphNodes) {
  ...
  const prev = dom.searchKind.value || wbState.searchKind || 'all';
  ...
  if (dom.searchQuery.value !== (wbState.searchQuery || '')) {
    dom.searchQuery.value = wbState.searchQuery || '';
  }

  const q = (wbState.searchQuery || '').trim().toLowerCase();
  ...
  const selType = normaliseSelectionType(wbState.selection?.type);
  const selectedNodeId =
    selType && selType !== 'relation' && selType !== 'edge' ? wbState.selection.id : null;
...
function renderGraphWorkbenchFilters(ctx, dom, baseGraph) {
  ...
  const filteredTypes = wbState.filterNodeTypes.filter(t => nodeTypes.includes(t));
  const typesChanged =
    filteredTypes.length !== wbState.filterNodeTypes.length ||
    filteredTypes.some((t, idx) => t !== wbState.filterNodeTypes[idx]);
  if (typesChanged) {
    patchWorkbenchState(ctx, { filterNodeTypes: filteredTypes });
    return;
  }

  if (wbState.filterCenterId && !nodeMap.has(wbState.filterCenterId)) {
    patchWorkbenchState(ctx, { filterCenterId: null });
    return;
  }

  const currentWbState = getWorkbenchState(ctx);
  const centerNode = currentWbState.filterCenterId
    ? nodeMap.get(currentWbState.filterCenterId)
    : null;
  ...
  if (dom.filterDepth) {
    const desired =
      currentWbState.filterDepth === null || currentWbState.filterDepth === undefined
        ? ''
        : String(currentWbState.filterDepth);
  ...
  const selectedTypes = new Set(currentWbState.filterNodeTypes);
  ...
  nodeTypes.forEach(type => {
    ...
    cb.addEventListener('change', () => {
      const nextTypes = cb.checked
        ? currentWbState.filterNodeTypes.concat(type).filter(Boolean)
        : currentWbState.filterNodeTypes.filter(t => t !== type);
...
function renderGenericNodeEditor(ctx, dom, node, config) {
  ...
  config.fields.forEach(field => {
    ...
    if (field.name === 'content') {
      ...
      act.className = 'markdown-row-actions';
  ...
  btnSave.addEventListener('click', () => {
    ...
    try {
      ...
      store?.saveSnapshot?.({ show: false });
      ui?.setStatus?.(`${config.label} saved`);
      renderGraphWorkbench(ctx);
    } catch (err) {
      if (typeof window !== 'undefined') {
  ...
  btnDelete.addEventListener('click', () => {
    ...
    try {
      ...
      patchWorkbenchState(ctx, { selection: null });
      store?.saveSnapshot?.({ show: false });
      ui?.setStatus?.(`${config.label} deleted`);
      renderGraphWorkbench(ctx);
    } catch (err) {
      if (typeof window !== 'undefined') {
...
function renderSelected(ctx, dom, visibleEntities, entityById, baseGraph) {
  const state = getState(ctx);
  const snapshot = state.snapshot || {};
  ...
  const selection = wbState.selection;
  if (!selection) {
...
export function renderGraphWorkbench(ctx) {
  ...
  dom.workbench?.style?.setProperty('--graph-left-width', `${wbState.leftWidth}px`);
  dom.workbench?.style?.setProperty('--graph-right-width', `${wbState.rightWidth}px`);

  const ViewModels = getViewModels(ctx);
  if (!ViewModels?.buildMovementGraphModel || !ViewModels?.filterGraphModel) return;
  ...
  const { visibleEntities, entityById } = getGraphDatasetForMovement(ctx);
  const entityIds = new Set(visibleEntities.map(e => e.id));

  if (wbState.selection) {
    const sel = wbState.selection;
    const selType = normaliseSelectionType(sel.type);
    let valid = true;
    if (selType === 'entity') {
      valid = baseNodeIds.has(sel.id) && entityIds.has(sel.id);
    } else if (selType === 'edge') {
      valid = baseEdgeIds.has(sel.id);
    } else {
      valid = baseNodeIds.has(sel.id);
    }
    if (!valid) {
      patchWorkbenchState(ctx, { selection: null });
      return;
  ...
  renderSelected(ctx, dom, visibleEntities, entityById, baseGraph);
  ...
  const filteredGraph = ViewModels.filterGraphModel(baseGraph, {
    centerNodeId: wbState.filterCenterId,
    depth: wbState.filterDepth,
    nodeTypeFilter: wbState.filterNodeTypes
  });

  const selectedType = normaliseSelectionType(wbState.selection && wbState.selection.type);

  const selectedNodeIdForGraph =
    wbState.selection && selectedType !== 'edge' ? wbState.selection.id : null;

  if (getWorkbenchState(ctx).focusEntityId !== wbState.filterCenterId) {
    patchWorkbenchState(ctx, { focusEntityId: wbState.filterCenterId });
  }

  workbenchGraphView?.render?.(dom.canvas, filteredGraph, {
    selectedEntityId: selectedNodeIdForGraph,
    selectedEdgeId: selectedType === 'edge' ? wbState.selection.id : null,
    focusEntityId: wbState.filterCenterId
  });
}
