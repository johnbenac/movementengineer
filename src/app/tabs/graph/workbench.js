...
const fallbackClear = el => {
  if (!el) return;
  while (el.firstChild) el.removeChild(el.firstChild);
};

const normaliseSelectionType = type => (typeof type === 'string' ? type.toLowerCase() : null);
const labelForNodeType = type => GRAPH_NODE_TYPE_LABELS[type] || type || 'Unknown';
const colorForNodeType = type =>
  (typeof window !== 'undefined' && window.EntityGraphColors?.colorForNodeType?.(type)) ||
  '#1f2937';
...
function getMarkdownUi(ctx) {
  const markdown = ctx?.ui?.markdown || movementEngineerGlobal?.ui?.markdown || {};
  return {
    renderMarkdownPreview: markdown.renderMarkdownPreview || renderMarkdownPreview,
    openMarkdownModal: markdown.openMarkdownModal || openMarkdownModal
  };
}

function setStatus(ctx, text) {
  if (ctx?.ui?.setStatus) ctx.ui.setStatus(text);
  else ctx?.store?.setStatus?.(text);
}

function getWorkbenchState(ctx) {
  const state = getState(ctx);
  return state?.graphWorkbenchState
    ? { ...DEFAULT_GRAPH_WORKBENCH_STATE, ...state.graphWorkbenchState }
...
function patchWorkbenchState(ctx, patch) {
  const patchObject =
    typeof patch === 'function'
      ? prev => patch(prev || DEFAULT_GRAPH_WORKBENCH_STATE)
      : () => patch;
  ...
  const apply = prev => ({
    ...
    graphWorkbenchState: {
      ...
      ...(patchObject(prev?.graphWorkbenchState || {}) || {})
    }
  });

  if (ctx?.store?.update) return ctx.store.update(apply);
  if (ctx?.store?.setState) return ctx.store.setState(apply);
  return null;
}
...
function getGraphDataset(ctx) {
  const state = getState(ctx);
  const movementId = state.currentMovementId;
  const snap = state.snapshot || {};
  const allEntities = normaliseArray(snap.entities);
...
function attachResize(handleEl, side, ctx, dom) {
  if (!handleEl) return;
  handleEl.addEventListener('mousedown', e => {
    e.preventDefault();
    const startX = e.clientX;
    const state = getWorkbenchState(ctx);
...
function ensureGraphWorkbenchDom(ctx) {
  ...
  if (graphWorkbenchDom && graphWorkbenchDom.root === root) {
    if (graphWorkbenchDom.workbench) {
      graphWorkbenchDom.workbench.style.setProperty(
        '--graph-left-width',
        `${workbenchState.leftWidth}px`
      );
      graphWorkbenchDom.workbench.style.setProperty(
        '--graph-right-width',
        `${workbenchState.rightWidth}px`
      );
    }
  ...
  attachResize(dom.leftHandle, 'left', ctx, dom);
  attachResize(dom.rightHandle, 'right', ctx, dom);
  ...
  dom.createEntityForm?.addEventListener('submit', e => {
    ...
    try {
      const nextSnapshot = cloneSnapshot(state.snapshot, StorageService);
      const entity = DomainService.addNewItem(nextSnapshot, 'entities', movementId);
      entity.name = name;
      entity.kind = kind;
      ...
      DomainService.upsertItem(nextSnapshot, 'entities', entity);
...
function renderGraphSearch(ctx, dom, baseGraphNodes) {
  ...
  const clearElement = getClear(ctx);
  ...
  filtered
    ...
    .forEach(node => {
      ...
      li.addEventListener('click', () => {
        ...
        patchWorkbenchState(ctx, { filterCenterId: node.id, focusEntityId: node.id });
      });
...
function renderGraphWorkbenchFilters(ctx, dom, baseGraph) {
  if (!dom) return { shouldRerender: false };
  const workbenchState = getWorkbenchState(ctx);
  const clearElement = getClear(ctx);
  ...
  const nextNodeTypes = workbenchState.filterNodeTypes.filter(t => nodeTypes.includes(t));
  const needsTypePatch = nextNodeTypes.length !== workbenchState.filterNodeTypes.length;

  const nextCenterId =
    workbenchState.filterCenterId && nodeMap.has(workbenchState.filterCenterId)
      ? workbenchState.filterCenterId
      : null;
  const needsCenterPatch = workbenchState.filterCenterId && !nextCenterId;

  if (needsTypePatch || needsCenterPatch) {
    patchWorkbenchState(ctx, {
      filterNodeTypes: nextNodeTypes,
      filterCenterId: nextCenterId || null,
      focusEntityId: nextCenterId || null
    });
    return { shouldRerender: true };
  }

  const centerNode = workbenchState.filterCenterId ? nodeMap.get(workbenchState.filterCenterId) : null;
  ...
  if (!dom.filterTypes) return { shouldRerender: false };
  ...
  nodeTypes.forEach(type => {
    ...
    cb.addEventListener('change', () => {
      if (cb.checked) {
        const merged = workbenchState.filterNodeTypes.includes(type)
          ? workbenchState.filterNodeTypes
          : workbenchState.filterNodeTypes.concat(type);
        patchWorkbenchState(ctx, { filterNodeTypes: merged });
      } else {
        patchWorkbenchState(ctx, {
...
function renderGenericNodeEditor(ctx, dom, node, config) {
  ...
  const DomainService = getDomainService(ctx);
  const StorageService = getStorageService(ctx);
  const markdownUi = getMarkdownUi(ctx);
  const clearElement = getClear(ctx);
  ...
  config.fields.forEach(field => {
    ...
    if (field.name === 'content') {
      ...
      actionsRow.className = 'markdown-row-actions';
      ...
      markdownUi.renderMarkdownPreview(preview, control.value, { enabled: true });
      ...
      openBtn.addEventListener('click', () => {
        ...
        markdownUi.openMarkdownModal({
          title: 'Edit canon text',
          initial: control.value,
          onSave: value => {
            control.value = value;
            markdownUi.renderMarkdownPreview(preview, value, { enabled: true });
          },
          onClose: () => {
            markdownUi.renderMarkdownPreview(preview, control.value, { enabled: true });
          }
  ...
  btnSave.addEventListener('click', () => {
    if (!DomainService?.upsertItem) {
      window.alert('DomainService is not available; cannot save this record.');
      return;
    }
    ...
    try {
      const nextSnapshot = cloneSnapshot(getSnapshot(ctx), StorageService);
      DomainService.upsertItem(nextSnapshot, config.collection, updated);
      ctx?.store?.setState?.(prev => ({ ...prev, snapshot: nextSnapshot }));
      ctx?.store?.saveSnapshot?.({ show: false });
  ...
  btnDelete.addEventListener('click', () => {
    ...
    if (!DomainService?.deleteItem) {
      window.alert('DomainService is not available; cannot delete this record.');
      return;
    }

    try {
      const nextSnapshot = cloneSnapshot(getSnapshot(ctx), StorageService);
      DomainService.deleteItem(nextSnapshot, config.collection, item.id);
      ctx?.store?.setState?.(prev => ({ ...prev, snapshot: nextSnapshot }));
      setGraphWorkbenchSelection(ctx, null);
      ctx?.store?.saveSnapshot?.({ show: false });
...
function renderSelected(ctx, dom, visibleEntities, entityById, baseGraph, workbenchState) {
  if (!dom) return;
  const clearElement = getClear(ctx);
  clearElement(dom.selectedBody);
  ...
  if (selectionType === 'entity') {
    ...
    actionWrap.appendChild(btnSave);
    ...
    const addInput = (label, name, value, opts) => {
      const row = document.createElement('div');
      row.className = 'form-row';
    ...
    const addTextarea = (label, name, value, rows) => {
      const row = document.createElement('div');
      row.className = 'form-row';
      const l = document.createElement('label');
      l.textContent = label;
    ...
    btnSave.addEventListener('click', () => {
      if (!DomainService?.upsertItem) {
        window.alert('DomainService is not available; cannot save this entity.');
        return;
      }
      const fd = new FormData(form);
      const updated = {
      ...
      try {
        const nextSnapshot = cloneSnapshot(getSnapshot(ctx), StorageService);
        DomainService.upsertItem(nextSnapshot, 'entities', updated);
        ctx?.store?.setState?.(prev => ({ ...prev, snapshot: nextSnapshot }));
        ctx?.store?.saveSnapshot?.({ show: false });
    ...
    btnDelete.addEventListener('click', () => {
      ...
      if (!DomainService?.deleteItem) {
        window.alert('DomainService is not available; cannot delete this entity.');
        return;
      }

      try {
        const nextSnapshot = cloneSnapshot(getSnapshot(ctx), StorageService);
        DomainService.deleteItem(nextSnapshot, 'entities', entity.id);
        ctx?.store?.setState?.(prev => ({ ...prev, snapshot: nextSnapshot }));
        patchWorkbenchState(ctx, {
          selection: null,
          filterCenterId:
            workbenchState.filterCenterId === entity.id ? null : workbenchState.filterCenterId,
          focusEntityId:
            workbenchState.focusEntityId === entity.id ? null : workbenchState.focusEntityId
        });
        ctx?.store?.saveSnapshot?.({ show: false });
...
export function mountGraphWorkbench() {
  graphWorkbenchDom = graphWorkbenchDom || null;
}
...
export function renderGraphWorkbench(ctx) {
  ...
  const baseGraph =
    ViewModels.buildMovementGraphModel(snapshot, {
  ...
  const { visibleEntities, entityById } = getGraphDataset(ctx);
  ...
  const filterResult = renderGraphWorkbenchFilters(ctx, dom, baseGraph);
  if (filterResult.shouldRerender) return;
  ...
  renderSelected(ctx, dom, visibleEntities, entityById, baseGraph, workbenchState);

  if (!workbenchGraphView) {
    const GraphView = getEntityGraphView(ctx);
    if (!GraphView) return;
    workbenchGraphView = new GraphView({
      onNodeClick: (id, node) => {
        const type = normaliseSelectionType(node?.type) || 'node';
        setGraphWorkbenchSelection(ctx, { type, id });
        patchWorkbenchState(ctx, { filterCenterId: id, focusEntityId: id });
      },
      onLinkClick: id => {
  ...
  const selectedNodeIdForGraph =
    workbenchState.selection && selectedType !== 'edge' ? workbenchState.selection.id : null;
