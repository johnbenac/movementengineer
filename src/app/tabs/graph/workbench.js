...
function getCurrentMovementId(ctx) {
  const state = getState(ctx);
...
function getViewModels(ctx) {
  return ctx?.services?.ViewModels || window.ViewModels;
}
...
function setStatus(ctx, text) {
  ...
  if (ctx?.setStatus) return ctx.setStatus(text);
}

function jumpToReferencedItem(ctx, collectionName, itemId) {
  if (ctx?.actions?.jumpToReferencedItem) {
    return ctx.actions.jumpToReferencedItem(collectionName, itemId);
  }
  const tab = movementEngineerGlobal?.tabs?.collections;
  if (tab?.jumpToReferencedItem) {
    return tab.jumpToReferencedItem(ctx, collectionName, itemId);
  }
  return null;
}
...
function getGraphDatasetForCurrentMovement(ctx, snapshot, movementId) {
...
function attachResize(handleEl, side, ctx, dom) {
  if (!handleEl || !dom?.workbench) return;
  handleEl.addEventListener('mousedown', e => {
    e.preventDefault();
    const startX = e.clientX;
    const current = getWorkbenchState(ctx);
    const startLeft = current.leftWidth;
    const startRight = current.rightWidth;
...
function ensureGraphWorkbenchDom(ctx) {
  ...
  dom.createEntityForm.addEventListener('submit', e => {
    e.preventDefault();
    const movementId = getCurrentMovementId(ctx);
    ...
    try {
      const entity = DomainService.addNewItem(snapshot, 'entities', movementId);
      entity.name = name;
      entity.kind = kind;
...
function renderGraphWorkbenchFilters(ctx, dom, baseGraph) {
  ...
  const filteredNodeTypes = (workbenchState.filterNodeTypes || []).filter(t => nodeTypes.includes(t));
  if (filteredNodeTypes.length !== (workbenchState.filterNodeTypes || []).length) {
    patchWorkbenchState(ctx, { filterNodeTypes: filteredNodeTypes });
    return true;
  }
  ...
  nodeTypes.forEach(type => {
    ...
    cb.addEventListener('change', () => {
      const currentTypes = getWorkbenchState(ctx).filterNodeTypes || [];
      if (cb.checked) {
        if (!currentTypes.includes(type)) {
          patchWorkbenchState(ctx, {
            filterNodeTypes: currentTypes.concat(type).filter(Boolean)
          });
        }
      } else {
        patchWorkbenchState(ctx, {
          filterNodeTypes: currentTypes.filter(t => t !== type)
        });
      }
      renderGraphWorkbench(ctx);
    });
...
function renderGenericNodeEditor(ctx, dom, snapshot, node, config) {
  ...
  btnSave.addEventListener('click', () => {
    ...
    try {
      getDomainService(ctx).upsertItem(snapshot, config.collection, updated);
      ctx?.store?.saveSnapshot?.({ show: false });
  ...
  btnDelete.addEventListener('click', () => {
    ...
    try {
      getDomainService(ctx).deleteItem(snapshot, config.collection, item.id);
      setGraphWorkbenchSelection(ctx, null);
      ctx?.store?.saveSnapshot?.({ show: false });
...
function renderSelected(ctx, dom, snapshot, visibleEntities, entityById, baseGraph) {
    ...
    if (edge.source) {
      ...
      if (edge.source.collection && edge.source.id) {
        ...
        btn.addEventListener('click', () =>
          jumpToReferencedItem(ctx, edge.source.collection, edge.source.id)
        );
  ...
  if (node) {
    ...
    if (config) {
      renderGenericNodeEditor(ctx, dom, snapshot, node, config);
      return;
    }
...
export function renderGraphWorkbench(ctx) {
  ...
  if (!ViewModels || typeof ViewModels.buildMovementGraphModel !== 'function') return;
  ...
  const baseGraph = ViewModels.buildMovementGraphModel(snapshot, { movementId });
  ...
  const filtersPatched = renderGraphWorkbenchFilters(ctx, dom, baseGraph);
  ...
  renderSelected(ctx, dom, snapshot, visibleEntities, entityById, baseGraph);
  ...
  const nextFocus = workbenchState.filterCenterId;
  if (nextFocus !== workbenchState.focusEntityId) {
...
export function unmountGraphWorkbench() {
  graphWorkbenchDom = null;
  if (workbenchGraphView?.destroy) {
    try {
      workbenchGraphView.destroy();
    } catch (err) {
      // ignore
    }
  }
