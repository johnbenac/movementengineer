...
export function getCollectionDoc(model, collectionName) {
  ...
  Object.entries(collection.fields || {}).forEach(([name, field]) => {
    ...
    if (field.type === 'array' && field.items?.ref) {
      const target = field.items.ref === '*' ? 'any' : field.items.ref;
      referenceFields.push({ field: name, target, kind: 'many' });
      return;
    }
    if (field.ref) {
      const target = field.ref === '*' ? 'any' : field.ref;
      referenceFields.push({ field: name, target, kind: 'one' });
    }
  });
