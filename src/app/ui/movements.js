...
export function initMovements(ctx, options = {}) {
  ...
  async function handleImportRepoClick() {
    ...
    try {
      ...
      const movements = Array.isArray(importedSnapshot?.movements)
        ...
        : [];
      const firstId =
        movements[0]?.id || movements[0]?.movementId || importedSnapshot?.currentMovementId || null;
  ...
  async function handleExportZipClick() {
    ...
    try {
      ...
      let snapshotForExport = snapshot;

      if (!snapshotForExport.__repoBaselineByMovement) {
        const baseline =
          typeof loader.buildBaselineByMovement === 'function'
            ? loader.buildBaselineByMovement(snapshotForExport)
            : {};
        snapshotForExport = {
          ...snapshotForExport,
          __repoBaselineByMovement: baseline,
          __repoFileIndex: snapshotForExport.__repoFileIndex || {},
          __repoRawMarkdownByPath: snapshotForExport.__repoRawMarkdownByPath || {}
        };
        if (ctx.store?.setState) {
          ctx.store.setState(prev => ({ ...prev, snapshot: snapshotForExport }));
        }
      }

      const result = await loader.exportMovementToZip(snapshotForExport, movementId, {
        outputType: 'blob'
      });
