(function () {
  ...
  function resolveExportSchema(collection, specVersion) {
    const model = getModel(resolveSpecVersion(specVersion));
    return model?.getExportSchema ? model.getExportSchema(collection) : null;
  }
  ...
  function generateMarkdownForRecord(collection, record, specVersion) {
    const schema = resolveExportSchema(collection, specVersion);
    if (!schema) {
  ...
  function renderMarkdownForRecord(snapshot, collection, record) {
    ...
    if (raw !== undefined && baselineRecord) {
      return patchMarkdown(raw, collection, baselineRecord, record, path, snapshot.specVersion);
    }

    return generateMarkdownForRecord(collection, record, snapshot.specVersion);
  }
