...
def main() -> None:
    ...
    try:
        ...
        successful_prs_without_logs: List[Tuple[Dict[str, str], str]] = []
        ...
        for pr_info in pr_infos:
            ...
            output_base = os.path.join(
                args.output_dir, f"pr-{pr_info['number']}-implementation"
            )
            outputs = [
                (f"{output_base}-without-logs.txt", False, successful_prs_without_logs),
                (f"{output_base}-with-logs.txt", True, successful_prs_with_logs),
            ]

            for output_file, include_logs, target_list in outputs:
                if run_big_picture(
                    pr_info,
                    existing_files,
                    comments,
                    ...
                    target_list.append((pr_info, output_file))

        compilation_sets = [
            ("without-logs", successful_prs_without_logs),
            ("with-logs", successful_prs_with_logs),
        ]

        for label, successful_prs in compilation_sets:
            if not successful_prs:
                continue

            master_output = os.path.join(
                args.output_dir,
                f"pr-comparison-{args.start_pr}-{args.end_pr}-{label}.txt",
            )
            ...
            summary_output = os.path.join(
                args.output_dir,
                f"pr-summaries-{args.start_pr}-{args.end_pr}-{label}.txt",
            )
            ...
            touched_output = os.path.join(
                args.output_dir,
                f"pr-touched-files-{args.start_pr}-{args.end_pr}-{label}.txt",
            )
            ...
            print(f"\n✓ Successfully processed {len(successful_prs)} PR(s) ({label})")
            print(
                f"✓ Individual files: {args.output_dir}/pr-{{num}}-implementation-{label}.txt"
            )
        ...
        if not (successful_prs_with_logs or successful_prs_without_logs):
