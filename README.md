...
- **Entity** is intentionally generic: beings, places, objects, and ideas all live here with a `kind`, summary, and sources-of-truth fields. Entities can cite other entities as sources, and their connections to other records (claims, events, texts, media) are derived from those records’ outbound references.
...
- **One-way links win.** Texts, practices, rules, events, claims, and media all point to entities, not vice versa. The graph view is derived from those outbound references, keeping editing predictable and avoiding cascades when you delete or duplicate things.
