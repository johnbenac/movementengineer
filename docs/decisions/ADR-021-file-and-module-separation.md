...
2. **Domain logic** lives in service modules: `src/core/viewModels.js` contains pure projection functions (ADR‑019); modules like `src/core/comparison/comparisonServices.js` implement comparison and template operations (ADR‑018).
...
- Use simple naming conventions (e.g. `*-model.js` for schemas, `*-services.js` for logic, `viewModels.js` for projections) to aid discovery.
