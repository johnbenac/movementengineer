...
2. **Domain logic** lives in service modules: `src/core/viewModels.js` contains pure projection functions (ADR‑019); files like `src/core/comparison/comparisonServices.js` implement comparison and template operations (ADR‑018).
...
- Use simple naming conventions (e.g. camelCase module names in `src/core/**` and `src/runtime/**`, JSON under `src/models/`) to aid discovery.
