...
function testGraphFiltering() {
  ...
  // Hop-only (depth 1 from TextNode)
  ...
  // Combined: show only Claim nodes but keep center
  const combined = ViewModels.filterGraphModel(base, {
    centerNodeId: 't1',
    depth: 2,
    nodeTypeFilter: ['Claim']
  });
  assert(combined.nodes.some(n => n.id === 't1'), 'Center preserved even if type-filtered out');
