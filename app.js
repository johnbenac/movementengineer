...
(function () {
  ...
  function doesCanonNodeMatchFilters(node, filters) {
    ...
    if (query) {
      const haystack = [
        ...
        node.label,
        node.mainFunction,
        typeof node.depth === 'number' ? `depth:${node.depth}` : null,
  ...
  function renderCanonView() {
    ...
    const renderNode = node => {
      ...
      const bits = [];
      if (typeof node.depth === 'number') bits.push(`depth ${node.depth}`);
  ...
  function renderNodeEditor(vm) {
    ...
    depthDisplay.className = 'muted';
    depthDisplay.textContent = `Depth ${activeNode.depth ?? 0} (derived)`;
    ...
    [
      ...
      { label: 'Depth (derived)', field: depthDisplay },
    ...
    if (vm.mentioningTexts && vm.mentioningTexts.length) {
      mkSection('Mentioned in texts', section => {
        ...
        vm.mentioningTexts.forEach(t => {
          ...
          chip.title = typeof t.depth === 'number' ? `Depth ${t.depth}` : '';
  ...
  function renderDashboard() {
    ...
    const depthEntries = Object.entries(vm.textStats.byDepth || {}).sort(
      ([a], [b]) => Number(a) - Number(b)
    );
    const depthSummary = depthEntries.length
      ? depthEntries.map(([d, c]) => `d${d}: ${c}`).join(' · ')
      : 'No texts yet.';
    textCard.innerHTML =
      '<h3>Texts</h3>' +
      `<p>Total: ${vm.textStats.totalTexts}</p>` +
      `<p>Max depth: ${vm.textStats.maxDepth ?? 0}</p>` +
      `<p>By depth: ${depthSummary}</p>`;
  ...
  function updateComparisonTable() {
    ...
    addMetricRow('Root texts (depth 0)', r => r.textCounts.rootCount ?? 0);
    addMetricRow('Max text depth', r => r.textCounts.maxDepth ?? 0);
    addMetricRow('Texts by depth', r => {
      const entries = Object.entries(r.textCounts.byDepth || {}).sort(
        ([a], [b]) => Number(a) - Number(b)
      );
      return entries.map(([d, c]) => `d${d}:${c}`).join(', ');
    });
