...
/* global DomainService, StorageService, ViewModels, GraphViewer, JSZip, d3 */
...
(function () {
  ...
  function renderEntitiesView() {
    ...
    if (!entityGraphView) {
      entityGraphView = new GraphViewer({
        onNodeClick: id => {
          if (!id) return;
  ...
  function renderGraphWorkbench() {
    ...
    if (!workbenchGraphView) {
      workbenchGraphView = new GraphViewer({
        onNodeClick: id => {
    ...
    const graphDataRaw = buildGraphNodesAndLinks(visibleEntities, visibleRelations);
    const graphData = {
      nodes: graphDataRaw.nodes,
      edges: graphDataRaw.links.map(l => ({
        id: l.id,
    ...
    workbenchGraphView.render(dom.canvas, graphData, {
      ...
      showSummary: true,
      height: dom.canvas.clientHeight || 420
    });
  }
