...
(function () {
  ...
  function createWorkbenchGraphView(options) {
    return new EntityGraphView({
      ...
      onEdgeClick: options.onLinkClick,
  ...
  function renderGraphWorkbench() {
    ...
    workbenchGraphView.render(dom.canvas, graphData, {
      selectedNodeId: selectedEntityIdForGraph,
      selectedEdgeId: selectedRelationIdForGraph,
      focusNodeId: graphWorkbenchState.focusEntityId,
      width: dom.canvas.clientWidth || undefined,
      height: Math.max(dom.canvas.clientHeight, 420)
    });
  }
