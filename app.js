...
(function () {
  ...
  function persistDirtyChanges() {
    ...
    if (itemEditorDirty) {
      const collectionsTab = movementEngineerGlobal.tabs?.collections;
      const saved = collectionsTab?.saveItemFromEditor
        ? collectionsTab.saveItemFromEditor({ persist: false })
        : false;
      if (!saved) return;
    }
  ...
  function renderActiveTab() {
    ...
    switch (tabName) {
      ...
      case 'data':
        showFatalImportError(
          new Error('Collections tab has been renamed. Please use the Collections tab.')
        );
        break;
      case 'comparison':
        showFatalImportError(
  ...
  function setCollectionAndItem(collectionName, itemId, options = {}) {
    ...
    if (tab?.setCollectionAndItem) {
      return tab.setCollectionAndItem(collectionName, itemId, options);
    }
    setStatus('Collections module unavailable.');
    return null;
  }
  ...
  function jumpToReferencedItem(collectionName, itemId) {
    ...
    if (tab?.jumpToReferencedItem) {
      tab.jumpToReferencedItem(collectionName, itemId);
      return;
    }
