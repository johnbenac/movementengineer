...
test('renders notes tab and filters by target type and id', async ({ page }) => {
  await page.goto('/');
...
test('shows empty state message when no notes exist for movement', async ({ page }) => {
  ...
  await expect(page.locator('#notes-table-wrapper')).toContainText('No notes match this filter.');
});

test('disables controls and shows hint when no movement is selected', async ({ page }) => {
  const emptySnapshot = {
    ...notesSnapshot,
    movements: [],
    notes: [],
