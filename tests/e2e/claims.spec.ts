...
test('renders claims tab via module override without fatal banner', async ({ page }) => {
  await page.goto('/');
...
test('filters claims by category and entity', async ({ page }) => {
  ...
  const category = page.locator('#claims-category-filter');
  const entity = page.locator('#claims-entity-filter');
  const rows = page.locator('#claims-table-wrapper table tr');

  await expect(rows).toHaveCount(3);

  await category.selectOption('Goal');
  await expect(rows).toHaveCount(2);

  await category.selectOption('Belief');
  await entity.selectOption('e1');
  await expect(rows).toHaveCount(2);
});
...
test('switching movements updates claims view', async ({ page }) => {
  ...
  await expect(rows).toHaveCount(2);
  await expect(page.locator('#claims-table-wrapper')).toContainText('Other movement claim');
});
