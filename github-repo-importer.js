(function () {
  ...
  // -------------------------------
  // CORS-safe GitHub import (NO ZIP)
  // -------------------------------
  ...
  // Resolve a ref (branch/tag) to a commit SHA, robust to:
  // - branch names with slashes
  // - GitHub "tree/<ref>/<path>" URLs (we trim until we find a real ref)
  async function resolveRefToCommitSha(owner, repo, requestedRef) {
    ...
    const derefTagToCommit = async sha => {
      // annotated tags can point to a tag object which points to a commit
      let currentSha = sha;
      for (let i = 0; i < 4; i += 1) {
    ...
    // If user pasted tree/<ref>/<path>, this might be "main/some/folder".
    // We try full string, then progressively trim suffixes until a ref exists.
    ...
    // If no ref provided, use default branch (fallback main/master)
  ...
  async function parseMovementFromRepo(owner, repo, commitSha, movementJsonPath, blobPaths, pathIndex) {
    ...
    // Sequential insert => deterministic duplicate detection
